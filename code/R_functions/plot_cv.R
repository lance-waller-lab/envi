plot_cv <- function(input, alpha = 0.05) {
  ...
  title(paste("log relative risk prediction\nAUC = ", round(out_cv_rr$cvAUC, digits = 3),
              " (95% CI: ", round(out_ci_rr$ci[1], digits = 3),
              " - ", round(out_ci_rr$ci[2], digits = 3), ")", sep = ""), cex.main = 1.1)
  ...
  title(paste("Asymptotic p-value prediction\nAUC = ", round(out_cv_pval$cvAUC, digits = 3),
  ...
  title("log relative risk prediction", cex.main = 1.1)
  ...
  suppressWarnings(lines(colMeans(do.call(rbind,perf_pval@x.values)),
        ...
        col = "red", lty = 1, lwd = 2)) # mean PRREC
  title("Asymptotic p-value prediction", cex.main = 1.1)
