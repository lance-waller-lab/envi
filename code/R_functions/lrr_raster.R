lrr_raster <- function(input, cols, midpoint, thresh_up = NULL, thresh_low = NULL) {
  ...
  min_absolute_value <- min(input@data@values[is.finite(input@data@values)], na.rm = T) # minimum absolute value of raster
  max_absolute_value <- max(input@data@values[is.finite(input@data@values)], na.rm = T) # maximum absolute value of raster
