...
# MASK to WUS
mask_ppt_rast_scale <- mask(scaled_reproj_ppt_rast,wus_buffer)
mask_tdmean_rast_scale <- mask(scaled_reproj_tdmean_rast,wus_buffer)
mask_tmax_rast_scale <- mask(scaled_reproj_tmax_rast,wus_buffer)
mask_tmean_rast_scale <- mask(scaled_reproj_tmean_rast,wus_buffer)
mask_tmin_rast_scale <- mask(scaled_reproj_tmin_rast,wus_buffer)
mask_vpdmax_rast_scale <- mask(scaled_reproj_vpdmax_rast,wus_buffer)
mask_vpdmin_rast_scale <- mask(scaled_reproj_vpdmin_rast,wus_buffer)

# Raster Stack for PCA
mask_rasters_scaled <- stack(mask_ppt_rast_scale, mask_tdmean_rast_scale,
                             mask_tmax_rast_scale,mask_tmean_rast_scale,mask_tmin_rast_scale,mask_vpdmax_rast_scale,mask_vpdmin_rast_scale)
...
pc1_scaled <- (pc1_b1 + abs(pc1_b1@data@min))
pc2_scaled <- (pc1_b2 + abs(pc1_b2@data@min))
#pc3_scaled <- pc1_b3 + abs(pc1_b3@data@min)

# Western US Extraction Points
extract_points_wus <- rasterToPoints(pc1_b2)
...
# Restructure for jitter_KnD function
raster_dat <- stack(pc1_scaled, pc1_scaled)
